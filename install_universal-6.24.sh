...
# Version: 6.24 (English localization, NVIDIA Recommended Driver Heuristic)
# ==============================================================================
...
if [ "$GPU_CHOICE" -eq 1 ]; then
    # Dynamically detect NVIDIA GPU Model
    ...
    # Determine recommended driver branch based on GPU model keywords
...
# ==============================================================================
# 2. BASE SYSTEM UPDATE (Crucial for Kernel Module Matching)
# ==============================================================================
echo "=========================================================================="
...
# Ensure Linux modules are loaded in memory BEFORE installing linux dependencies
...
# Start the linux service immediately
...
# ==============================================================================
# 8. HARDWARE RULES (DEVFS) & PRINTERS
# ==============================================================================
...
case "$GPU_CHOICE" in
    1)
        ...
        echo "🟢 Installing NVIDIA proprietary driver (${NVIDIA_BRANCH})..."
        # Crucial fix: Do NOT install nvidia-settings or nvidia-xconfig to prevent branch conflicts.
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "🔒 NVIDIA driver packages have been successfully locked."
        fi
        ;;
    2)
...
# ==============================================================================
# 10. DESKTOP ENVIRONMENT & SDDM
# ==============================================================================
STARTWM_EXEC=""
case "$DE_CHOICE" in
    1)
        pkg install -y sddm pavucontrol kate konsole ark dolphin Kvantum plasma6-plasma kf6-frameworks qt6-5compat
...
# SDDM General Configuration & X11 enforcement
if [ "$DE_CHOICE" -ne 4 ]; then
    echo "⌨️  Configuring SDDM (Enforcing X11 Display Server)..."
    
    mkdir -p /usr/local/etc/sddm.conf.d
    cat > /usr/local/etc/sddm.conf.d/10-x11.conf << EOF
[General]
DisplayServer=x11
EOF
...
EOF
    ...
    # Ensure SDDM user can access DRM rendering
...
if [ "$DE_CHOICE" -ne 4 ] && [ "$THEME_NASA" -eq 0 ]; then
    ...
    # ------------------------------------------------------------------
    # FIX FOR QT6 CRASH: Update QML imports in Main.qml for Qt6 compatibility
    # ------------------------------------------------------------------
    echo "🔧 Patching Main.qml imports for Qt6 compatibility..."
...
EOF
    ...
    # FreeBSD vt(4) kernel splash screen requires PNG format
...
# ==============================================================================
# 11. METAPACKAGES (Internet, Fonts, Media, Vbox, etc.)
# ==============================================================================
