...
# Mode : Quarterly (Stable) + SDDM patché compilé depuis les Ports
# ==============================================================================
...
echo "=== Mise à jour du système de base et des paquets (Quarterly) ==="
...
# NOTE : SDDM est volontairement exclu d'ici pour être compilé plus bas
...
echo "=== Installation de KDE Plasma et Wayland ==="
...
echo "=== Compilation Hybride (NVIDIA, Qt6-Wayland & SDDM Patché) ==="
[ ! -d /usr/ports/.git ] && git clone --depth 1 https://git.freebsd.org/ports.git /usr/ports
# Mise à jour des ports pour garantir d'avoir le patch d'août 2026
...
if [ -f "$FLAG_FILE" ]; then
    ...
    echo "-> Début de la compilation ciblée (NVIDIA + Qt6-Wayland + SDDM)... Patientez."
    ...
    # Compilation du SDDM patché depuis les sources
    echo "-> Compilation de SDDM (avec le correctif Wayland TTY)..."
    pkg delete -y -f sddm >/dev/null 2>&1 || true
    cd /usr/ports/x11/sddm
    make BATCH=yes install clean >> "$LOG_FILE" 2>&1
...
# Autoriser SDDM à accéder à la carte graphique
pw groupmod video -m sddm

echo "=== Configuration de SDDM pour Plasma Wayland + NVIDIA ==="
mkdir -p /usr/local/share/sddm/scripts
cat << 'EOF' > /usr/local/share/sddm/scripts/wayland-session
#!/bin/sh
...
EOF
chmod +x /usr/local/share/sddm/scripts/wayland-session
...
echo "Note : Au redémarrage, SDDM s'affichera. Assurez-vous de sélectionner 'Plasma (Wayland)'."