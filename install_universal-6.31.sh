...
# Version: 6.31 (English localization, Wayland fixes, Gershwin Desktop Added)
# ==============================================================================
...
- Custom compilation of SDDM/Qt6 if required for Wayland stability.\n\n\
...
# NVIDIA Sub-menu with Hardware Detection Heuristic
NVIDIA_BRANCH=""
if [ "$GPU_CHOICE" -eq 1 ]; then
    NVIDIA_MODEL=$(pciconf -lv | grep -i -A 2 "vendor.*'NVIDIA" | grep "device" | cut -d"'" -f2 | head -n 1)
    [ -z "$NVIDIA_MODEL" ] && NVIDIA_MODEL="Unknown NVIDIA GPU"
    
    RECOMMENDED_DRIVER="latest"
    if echo "$NVIDIA_MODEL" | grep -qiE "RTX 4000|P2000|P4000|GTX 10|GTX 16|RTX 20|TITAN"; then
        RECOMMENDED_DRIVER="580"
    elif echo "$NVIDIA_MODEL" | grep -qiE "GTX 9|GTX 8|Quadro M"; then
        RECOMMENDED_DRIVER="470"
    elif echo "$NVIDIA_MODEL" | grep -qiE "GTX 7|GTX 6|Quadro K"; then
        RECOMMENDED_DRIVER="390"
    fi
    
    bsddialog --title "NVIDIA Driver Version" \
              --menu "Detected GPU: ${NVIDIA_MODEL}\nRecommended Driver: ${RECOMMENDED_DRIVER}\n\nSelect the NVIDIA driver branch for your GPU:" 18 75 6 \
...
bsddialog --title "Software Selection" \
          --checklist "Choose the components and applications to install:" 20 75 7 \
          "INTERNET" "Firefox, Chromium, Thunderbird" ON \
          "FONTS" "Supplementary system fonts and emojis" ON \
...
# ==============================================================================
# 4. SYSTEM OPTIMIZATIONS (Core & Network)
# ==============================================================================
...
pkg install -y xorg xauth xinit xterm consolekit2
...
case "$GPU_CHOICE" in
    1)
        ...
        pkg install -y "$NV_PKG"
        pkg install -y "$NV_LINUX_PKG" || true
        ...
        if [ $? -eq 0 ]; then
            pkg lock -q -y "$NV_PKG" 2>/dev/null || true
            pkg lock -q -y "$NV_LINUX_PKG" 2>/dev/null || true
        fi
        ;;
    2)
        pkg install -y drm-kmod gpu-firmware-amd-kmod wayland xwayland
        sysrc kld_list+="amdgpu"
        
        # STRICT AMDGPU BINDING FOR GCN 1.0/1.1 (Wayland Support)
...
# ==============================================================================
# 10. DESKTOP ENVIRONMENT & WAYLAND FIXES
# ==============================================================================
STARTWM_EXEC=""
case "$DE_CHOICE" in
    1)
        pkg install -y sddm pavucontrol kate konsole ark dolphin Kvantum plasma6-plasma kf6-frameworks qt6-5compat qt6-wayland
        sysrc sddm_enable="YES"
        STARTWM_EXEC="exec startplasma-x11"
        
        # ----------------------------------------------------------------------
        # COMPILE QT6-WAYLAND & SDDM FROM PORTS (Resolves 15.1-STABLE crash)
        # ----------------------------------------------------------------------
        FLAG_FILE="/root/.sddm_wayland_compiled"
        if [ ! -f "$FLAG_FILE" ]; then
            echo "=========================================================================="
            echo "🔧 Compiling Qt6-Wayland & SDDM from ports to fix Plasma Wayland mismatch..."
            echo "=========================================================================="
            [ ! -d /usr/ports/Makefile ] && git clone --depth 1 https://git.freebsd.org/ports.git /usr/ports
            git -C /usr/ports pull
            
            grep -q "OPTIONS_SET.*WAYLAND" /etc/make.conf 2>/dev/null || echo "OPTIONS_SET=WAYLAND EGL" >> /etc/make.conf
            
            pkg delete -y -f qt6-wayland >/dev/null 2>&1 || true
            make -C /usr/ports/graphics/qt6-wayland BATCH=yes CMAKE_ARGS+="-DQT_NO_PACKAGE_VERSION_CHECK=TRUE" install clean
            
            pkg delete -y -f sddm >/dev/null 2>&1 || true
            make -C /usr/ports/x11/sddm BATCH=yes install clean
    ...
    4)
        # ----------------------------------------------------------------------
        # GERSHWIN DESKTOP (GNUstep / Mac-like Environment)
        # ----------------------------------------------------------------------
        echo "=========================================================================="
        echo "🍎 Installing Gershwin Desktop & GNUstep Components..."
        echo "=========================================================================="
        pkg install -y sddm pavucontrol xfce4-wm
        
        # If the binary package exists in current repos, it installs cleanly.
        # Otherwise, gracefully fallback to the GitHub compilation script.
        if ! pkg install -y gershwin-desktop; then
            echo "-> Binary package not found in repos. Building Gershwin from source..."
            pkg install -y git gmake llvm clang curl bash
            [ ! -d /usr/local/src/gershwin-desktop ] && git clone https://github.com/gershwin-desktop/gershwin-desktop /usr/local/src/gershwin-desktop
            cd /usr/local/src/gershwin-desktop
            ./gershwin-build
        fi
        
        sysrc sddm_enable="YES"
        STARTWM_EXEC="exec gershwin-session"
        ;;
esac
...
EOF
    ...
    # KWIN_DRM_USE_MODIFIERS=0 fixes "horizontal lines" graphical corruption
    # eval exec "$@" correctly parses complex commands with arguments
    # ----------------------------------------------------------------------
    cat > /usr/local/share/sddm/scripts/wayland-session << EOF
#!/bin/sh
...
# Plasma 6 native DBus needs a clean environment.
# Only set XDG_RUNTIME_DIR if SDDM hasn't already.
...
# Use eval to properly split the command string passed by SDDM
eval exec "\$@" > /tmp/plasma-wayland.log 2>&1
EOF
    chmod 555 /usr/local/share/sddm/scripts/wayland-session

    mkdir -p /usr/local/etc/sddm.conf.d
    cat > /usr/local/etc/sddm.conf.d/wayland.conf << 'EOF'
[Wayland]
...
if [ "$DE_CHOICE" -ne 5 ] && [ "$THEME_NASA" -eq 0 ]; then
    ...
    sed -i '' 's/import QtQuick 2.*/import QtQuick/' /usr/local/share/sddm/themes/nasa/Main.qml
    sed -i '' 's/import QtQuick.Controls 1.*/import QtQuick.Controls/' /usr/local/share/sddm/themes/nasa/Main.qml
    sed -i '' 's/import QtQuick.Controls 2.*/import QtQuick.Controls/' /usr/local/share/sddm/themes/nasa/Main.qml
    sed -i '' 's/import QtQuick.Window 2.*/import QtQuick.Window/' /usr/local/share/sddm/themes/nasa/Main.qml
    sed -i '' 's/import QtGraphicalEffects.*/import Qt5Compat.GraphicalEffects/' /usr/local/share/sddm/themes/nasa/Main.qml
    ...
    if ! grep -q "^QtVersion=" /usr/local/share/sddm/themes/nasa/metadata.desktop; then
        echo "QtVersion=6" >> /usr/local/share/sddm/themes/nasa/metadata.desktop
    fi
...
if echo "$APP_CHOICES" | grep -q "FONTS"; then
    echo "🔤 Installing supplementary fonts..."
    pkg install -y cantarell-fonts droid-fonts-ttf noto-basic nerd-fonts
fi
...
# ==============================================================================
# 12. SYSTEM UPDATE (BRANCH SWITCH)
# ==============================================================================
